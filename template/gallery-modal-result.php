...
?>
<section id="modalGalleryResult" class="modal" style="display:none;">
    <div class="modal-content-gallery-error">
        <div class="gallery-images__modal-close">
            <a class="close" onclick="hideGalleryModelResult()"><img class="img__del"
                    src='/inc/assets/img/closemodalwhite.svg'></a>
        </div>
        <div class="gallery-images__modal-result-text">
            <span id="gallery-modal-result__text" class="gallery-modal-result__text"></span>
        </div>
                ...
                <div class="sub-block-inner" style="background-color:<?= $option['color'] ?>;">
                    <div class="sub-block-inner-title">
                        ...
                        <?php if($option['amount']!=1){?>
                            <div class="gallery-images__modal-sub-item-price-mounth"><?= round($option['price']/$option['amount'], 2) ?> <?= $fs['main_currency'] ?>/<?= $fs['Months'] ?></div>
        ...
        <div>
            <button class="pers_acc__profile-data-submit" onclick="window.location='/account-balance.php'">Top-up</button>
        </div>
    </div>
</section>